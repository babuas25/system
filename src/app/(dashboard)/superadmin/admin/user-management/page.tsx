...
export default function SuperAdminUserManagement() {
  ...
  const isRoleType = (value: string): value is RoleType =>
    (roleOptions as readonly string[]).includes(value)
  ...
  const handleDeleteUser = async (targetUser: UserDocument) => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete user "${targetUser.email}"? This action cannot be undone.`,
      )
    ) {
      return
    }
    ...
    try {
      ...
      setUsers((prev) =>
        prev.map((u) => {
          ...
          const mergedAvatar = editForm.profile?.avatar ?? u.profile.avatar
          const updatedProfile: UserDocument['profile'] = {
            firstName: editForm.profile?.firstName ?? u.profile.firstName,
            lastName: editForm.profile?.lastName ?? u.profile.lastName,
            gender: editForm.profile?.gender ?? u.profile.gender,
            dateOfBirth: editForm.profile?.dateOfBirth ?? u.profile.dateOfBirth,
            mobile: editForm.profile?.mobile ?? u.profile.mobile,
            avatar: mergedAvatar ?? '',
          }
          const updatedMetadata: UserDocument['metadata'] = {
            createdAt: editForm.metadata?.createdAt ?? u.metadata.createdAt,
            lastLoginAt: editForm.metadata?.lastLoginAt ?? u.metadata.lastLoginAt,
            isActive: editForm.metadata?.isActive ?? u.metadata.isActive,
            emailVerified: editForm.metadata?.emailVerified ?? u.metadata.emailVerified,
          }
          return {
            ...u,
                          ...
                          <div className="space-y-4">
                            <div className="grid grid-cols-1 gap-4">
                              ...
                              <div className="flex justify-between items-center py-2 border-b border-white/10">
                                ...
                                <span
                                  className={`text-sm font-medium ${user.metadata?.emailVerified ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                                >
                                  {user.metadata?.emailVerified ? 'Yes' : 'No'}
                                </span>
                              </div>
